...
// Cap value at 255 to prevent overflow
...
void sepia(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            ...
            // Compute sepia values using the sepia formula
...
{
    // Create a copy of the image to avoid modifying the original while computing
    ...
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            ...
            // Calculate the average for each color and update the pixel
