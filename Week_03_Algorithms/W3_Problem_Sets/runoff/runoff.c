...
int main(int argc, string argv[])
{
    ...
    // Run the election until a winner is found
    while (true)
    {
        ...
        // If tie, all remaining candidates win
        if (tie)
        {
            for (int i = 0; i < candidate_count; i++)
            {
...
int find_min(void)
{
    ...
    {
        if (!candidates[c].eliminated && candidates[c].votes < min_votes)
        {
...
bool is_tie(int min)
{
    ...
    return tie_count == remaining_candidates;
}
